...
class MenuWindow(QMainWindow):
    def __init__(self):
        ...
        self._initialize_search_settings()
    ...
    def _setup_connections(self):
        ...
        self.bt_eliminar.clicked.connect(self.vaciar_celda)
        self.bt_eliminar.clicked.connect(self.eliminar_registro)
        self.bt_insert.clicked.connect(self.insertar_datos)
        self.bt_actualizar1 = QtWidgets.QPushButton("Actualizar")
        self.bt_actualizar1.clicked.connect(self.actualizar_registro)
    ...
    def _setup_table_widget(self):
        ...
        self.bt_actualizar.clicked.connect(self.actualizar_registro_por_id)

    def _initialize_search_settings(self):
    ...
    def eliminar_registro(self):
        current_row = self.tableWidget.currentRow()
        if current_row < 0:
            self.statusBar().showMessage("Por favor selecciona un registro para eliminar")
            return
        row_id = self.tableWidget.item(current_row, 0).text()  # Suponiendo que la primera columna es el ID
        query = f"DELETE FROM empleados WHERE ID = {row_id}"
        try:
            fetch_all_data(query)  # Llamada para ejecutar la consulta
            self.statusBar().showMessage("Registro eliminado correctamente")
            self.actualizar_datos()
        except Exception as e:
            self.statusBar().showMessage(f"Error al eliminar registro: {e}")
    ...
    def actualizar_registro(self):
        current_row = self.tableWidget.currentRow()
        if current_row < 0:
            self.statusBar().showMessage("Por favor selecciona un registro para actualizar")
            return
        row_id = self.tableWidget.item(current_row, 0).text()  # Suponiendo que la primera columna es el ID
        new_data = {}
        for column in range(self.tableWidget.columnCount()):
            header = self.tableWidget.horizontalHeaderItem(column).text()
            cell_value = self.tableWidget.item(current_row, column).text()
            new_data[header] = cell_value
        set_clause = ", ".join([f"{key} = '{value}'" for key, value in new_data.items()])
        query = f"UPDATE empleados SET {set_clause} WHERE ID = {row_id}"
        try:
            fetch_all_data(query)
            self.statusBar().showMessage("Registro actualizado correctamente")
        except Exception as e:
            self.statusBar().showMessage(f"Error al actualizar el registro: {e}")

            self.statusBar().showMessage(f"Error al buscar datos: {e}")

    def actualizar_registro_por_id(self):
        record_id = self.searchbar.text()
        ...
        for column in range(self.tableWidget.columnCount()):
            header = self.tableWidget.horizontalHeaderItem(column).text()
            input_widget = self.findChild(QLineEdit, header)
            if input_widget and input_widget.text():
                modificaciones[header] = input_widget.text()
        if not modificaciones:
            self.statusBar().showMessage("No se encontraron modificaciones para guardar")
            return
        update_clause = ", ".join([f"{key} = '{value}'" for key, value in modificaciones.items()])
        query = f"UPDATE empleados SET {update_clause} WHERE ID = {record_id}"
        try:
            fetch_all_data(query)
            self.statusBar().showMessage("Registro modificado correctamente")
        except Exception as e:
            self.statusBar().showMessage(f"Error al actualizar el registro: {e}")
        self.setup_search()
